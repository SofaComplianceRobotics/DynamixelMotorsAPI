...
import dynamixelmotorsapi._dynamixelmotorsparameters as robotparameters
from dynamixelmotorsapi._logging_config import logger

class DynamixelMotors(ABC):
    """
    Abstract class to control Dynamixel motors.
    The class is designed to be used with the Dynamixel motors plugged into a FTDI USB hub.
    The motors are controlled in position mode. The class is thread-safe and can be used in a multi-threaded environment.

    Example:
        ```python
        from robotsmotorsapi import DynamixelMotors
        from robotsmotorsapi._dynamixelmotorsparameters import DXL_IDs
        from robotsmotorsapi._logging_config import logger
        ...
        # Open connection to the motors (optionally specify device name, or it will take the first FTDI device)
        if motors.open():
    ...
    _FTDI_list = {}  # Dict of all FTDI devices connected to the computer
    _initialized: bool = False

    # To be defined by child class
    _length_to_rad: float = None
    _rad_to_pulse: int = None
    _pulse_center: int = None
    _max_vel: float = None

    _length_to_pulse: int = None
    ...
    def __init__(self):
        self._lock = Lock()
        self._goal_velocity = field(default_factory=lambda: [0] * len(robotparameters.DXL_IDs))
        self._goal_position = field(default_factory=lambda: [0] * len(robotparameters.DXL_IDs))

        if not self._initialized:
            self._mg = motorgroup.MotorGroup(robotparameters)
            self._initialized = True
        
        message = "A child class of DynamixelMotors must initialize this parameter"
        assert self._length_to_rad is not None, message
        assert self._rad_to_pulse is not None, message
        assert self._pulse_center is not None, message
        assert self._max_vel is not None, message

        self._length_to_pulse = self._length_to_rad * self._rad_to_pulse
    ...
    def open(self, device_name: str=None, multi_turn: bool=False) -> bool:
        ...
        Args:
            device_name: str: if set, it will connect to the device with the given name, If not set, the first device will be used.
    ...
    def findAndOpen(self, device_name: str=None, multi_turn: bool=False) -> int:
        """
        Iterate over the serial ports and try to conenct to the first available FTDI device.
    ...
    def device_index(self) -> int:
        """Get the index of the device in the list of Robot Motors Devices from Robot MotorsAPI"""
